#include "GamusinoLineGeometry.h"
...
template <>
InputParameters
validParams<GamusinoLineGeometry>()
{
...
GamusinoLineGeometry::GamusinoLineGeometry(const InputParameters & parameters)
    ...
    _point_file(getParam<std::string>("point_file"))
{
  ...
  if (!file.good())
    mooseError("GamusinoLineGeometry: Error opening file " + _point_file);
  ...
  for (unsigned int i = 0; i + 1 < _xs.size(); ++i)
  {
    ...
    if (_half_seg_len[i] == 0)
      mooseError("GamusinoLineGeometry: zero-segment length detected at (x,y,z) = ",
                 _xs[i],
                 " ",
                 _ys[i],
                 " ",
...
bool
GamusinoLineGeometry::parseNextLineReals(std::ifstream & ifs, std::vector<Real> & myvec)
...
void
GamusinoLineGeometry::addPoints()
{
