...
template <>
InputParameters
validParams<GamusinoMaterialMInelastic>()
{
  InputParameters params = validParams<GamusinoMaterialMElastic>();
    ...
    _num_models(getParam<std::vector<MaterialName>>("inelastic_models").size())
{
  ...
  for (unsigned int i = 0; i < _num_models; ++i)
  {
    ...
    if (base)
      ...
      mooseError("GamusinoMaterialMInelastic: the inelastic model " + models[i] +
...
GamusinoMaterialMInelastic::updateQpStress(RankTwoTensor & combined_inelastic_strain_increment)
{
  ...
  if (counter == _max_its && l2norm_delta_stress > _absolute_tolerance &&
      (l2norm_delta_stress / first_l2norm_delta_stress) > _relative_tolerance)
    throw MooseException("GamusinoMaterialMInelastic: Max stress iteration hit!");
