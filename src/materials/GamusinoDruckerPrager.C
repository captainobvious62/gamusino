...
template <>
InputParameters
validParams<GamusinoDruckerPrager>()
{
  InputParameters params = validParams<GamusinoPQPlasticity>();
  ...
  params.addParam<MooseEnum>("MC_interpolation_scheme",
                             GamusinoDruckerPrager::MC_interpolation_scheme() = "DP2",
...
GamusinoDruckerPrager::GamusinoDruckerPrager(const InputParameters & parameters)
  : GamusinoPQPlasticity(parameters),
    _MC_cohesion(getUserObject<GamusinoHardeningModel>("MC_cohesion")),
    _MC_friction(getUserObject<GamusinoHardeningModel>("MC_friction")),
    _MC_dilation(getUserObject<GamusinoHardeningModel>("MC_dilation")),
    ...
    _smoother2(std::pow(getParam<Real>("smoother"), 2))
{
  if (_MC_friction.value(0.0) < 0.0 || _MC_dilation.value(0.0) < 0.0 ||
    ...
    mooseError("GamusinoDruckerPrager: both friction and dilation angles must lie in [0, Pi/2]!");
  if (_MC_friction.value(0.0) < _MC_dilation.value(0.0))
    mooseError("GamusinoDruckerPrager: friction angle should not be smaller than dilation angle");
  if (_MC_cohesion.value(0.0) < 0.0)
    mooseError("GamusinoDruckerPrager: cohesion should not be negative");
