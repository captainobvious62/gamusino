...
GamusinoMaterialMElastic::setBackgroundStress()
{
  ...
  if (!(num == 0 || num == 3))
    mooseError("Either zero or 3 background stress functions must be provided to GamusinoMaterialM*. "
               "You supplied ",
               num,
               ".\n");
...
GamusinoMaterialMElastic::computeQpProperties()
{
  ...
  {
    // TM coupling
    GamusinoMatPropertiesTM();
    GamusinoKernelPropertiesTM();
    GamusinoKernelPropertiesDerivativesTM();
  }
  else if ((_has_T) && (_has_pf))
  {
    // THM coupling
...
GamusinoMaterialMElastic::GamusinoMatPropertiesHM()
{
  ...
  // Bulk Density
  _bulk_density[_qp] = _porosity[_qp] * _fluid_density[_qp] + (1.0 - _porosity[_qp]) * _rho0_s;
}
