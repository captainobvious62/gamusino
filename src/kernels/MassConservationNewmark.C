...
MassConservationNewmark::MassConservationNewmark(const InputParameters & parameters)
  ...
  _grad_uy_old(_mesh.dimension() >= 2 ? coupledGradientOld("displacements",1) : _grad_zero),
  _grad_uz_old(_mesh.dimension() == 3 ? coupledGradientOld("displacements",2) : _grad_zero),
...
{
  // Derivative with respect to p is zero
  return 0.0;
}
